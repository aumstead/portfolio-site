...
function TimelineFigure5({ onMobile }) {
  const myTimeline = useRef(null);

  useEffect(() => {
    myTimeline.current = gsap.timeline();
    // change ids so there are no collisions on the page
    myTimeline.current
      .to("#gear-group7", {
        duration: 1,
        rotate: 360,
      ...
      .to("#red-icon-group7", { duration: .2, x: 45 }, "<")
      .to("#green-icon-group7", { duration: .2, y: -48 }, "<.2")
      .to("#blue-icon-group7", { duration: .2, y: 48 }, "<.2")
      .to("#yellow-icon-group7", { duration: .2, y: -48 }, "<.2")
...
export default TimelineFigure5;
